...
fn rect_becomes_path() {
    let out = conv(
        r##"<svg viewBox="0 0 24 24"><rect x="2" y="3" width="10" height="8" fill="#f00"/></svg>"##,
    );
...
fn rounded_rect_uses_arcs_converted_to_cubics() {
    let out = conv(
        r##"<svg viewBox="0 0 24 24"><rect x="0" y="0" width="20" height="20" rx="4" fill="#000"/></svg>"##,
    );
    assert!(out.contains("<path"));
    assert!(
        out.contains("C"),
        "rounded rect should emit cubic curves: {out}"
    );
}
...
fn circle_becomes_path_with_curves() {
    let out =
        conv(r##"<svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" fill="#00f"/></svg>"##);
    assert!(out.contains("<path"));
...
fn polygon_closes() {
    let out =
        conv(r##"<svg viewBox="0 0 24 24"><polygon points="0,0 10,0 5,10" fill="#0f0"/></svg>"##);
...
fn polyline_does_not_close() {
    let out = conv(
        r##"<svg viewBox="0 0 24 24"><polyline points="0,0 10,0 5,10" stroke="#000" stroke-width="1" fill="none"/></svg>"##,
    );
    let d = path_data(&out);
...
fn line_becomes_path() {
    let out = conv(
        r##"<svg viewBox="0 0 24 24"><line x1="0" y1="0" x2="10" y2="10" stroke="#000" stroke-width="2"/></svg>"##,
    );
...
fn arc_command_in_path_converts_to_cubics() {
    let out = conv(
        r##"<svg viewBox="0 0 24 24"><path d="M2 2 A 10 10 0 0 1 22 22" stroke="#000" fill="none"/></svg>"##,
    );
    let d = path_data(&out);
...
fn smooth_cubic_s_expands_to_c() {
    let out = conv(
        r##"<svg viewBox="0 0 24 24"><path d="M0 0 C 1 1 2 2 3 3 S 5 5 6 6" fill="#000"/></svg>"##,
    );
    let d = path_data(&out);
...
fn smooth_quad_t_expands_to_q() {
    let out =
        conv(r##"<svg viewBox="0 0 24 24"><path d="M0 0 Q 1 1 2 2 T 4 4" fill="#000"/></svg>"##);
    let d = path_data(&out);
...
fn degenerate_zero_size_rect_skipped() {
    ...
    let out = conv(
        r##"<svg viewBox="0 0 24 24"><rect x="0" y="0" width="0" height="10"/><circle cx="5" cy="5" r="3"/></svg>"##,
    );
